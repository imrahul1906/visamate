...
// Reads applicantName, passportNo, travelStartDate, travelDuration, cities from ApplicantContext.
// Writes them back on every change so other widgets (CoverLetterWidget) stay in sync.
...
export default function ItineraryWidget({
  ...
  // Use local state for text/date inputs so context re-renders don't swallow keystrokes.
  // Values are synced back to context onBlur (same pattern as daysInput below).
  ...
  // `days` is the committed number used for rendering day tabs and the PDF.
  // `daysInput` is the raw string the user is typing so the input doesn't snap back mid-edit.
  // Both use local state so changes reflect immediately without waiting for context re-render.
  ...
  /* ── Sync unique city names → context so CoverLetterWidget reads ctx.cities ── */
  useEffect(() => {
  ...
  /* ── Actions (delegated to itineraryService) ── */
  ...
  /* ── Download blank official format (delegated to itineraryService) ── */
  ...
  const generatePdf = async () => {
    ...
    try {
      ...
      // Word-wrap helper: injected measureText keeps this pure — uses service function
      const wrapLine = (text: string, maxW: number, size: number): string[] =>
        wrapText(text, maxW, (t) => timesRoman.widthOfTextAtSize(t, size));
      ...
      const rowsData: RowData[] = allDaysList.map((d, idx) => {
        ...
        const dateVal = dateForDayLocal(d);

        // Activities as bullet lines
        const actLines: string[] = [];
        for (const place of places) {
          const wrapped = wrapLine(place, innerWs[1] - 8, fontSize);
        ...
        const conVal = resolveRepeatedValue(accom.hotelContact || "", prevAccom?.hotelContact);
        const accVal = resolveRepeatedValue(accom.hotelName || "", prevAccom?.hotelName);

        const dateLines = wrapLine(dateVal, innerWs[0], fontSize);
        const conLines = wrapLine(conVal, innerWs[2], fontSize);
        const accLines = wrapLine(accVal, innerWs[3], fontSize);
...
rowsData.forEach((row) => {
  ...
  curY = rowBottom;
      ...
      // ── Save ──
      const filename = buildPdfFilename(countryName);
      ...
      // Sync itinerary cities back to context so CoverLetterWidget can use them
      const uniqueCities = getUniqueCityNames(itinerary, cities);
      if (uniqueCities.length > 0) update({ cities: uniqueCities });
    } catch (e) {
          ...
          <div className="iw-select-inner">
            ...
            <div className="iw-options">
              ...
              <button
                ...
                onClick={(e) => { e.stopPropagation(); handleDownloadOfficialPdf(); }}
              >
              ...
              <div className="iw-config-row">
                ...
                <div className="iw-config-field iw-config-field--sm">
                  ...
                  <input
                    ...
                    max={30}
                    onChange={(e) => {
                      // Always update raw string so user can freely edit (no snapping)
                    ...
                    onBlur={(e) => {
                      // On blur, clamp and commit to a valid value
                      const v = parseInt(e.target.value, 10);
                      if (isNaN(v) || v < 1) {
                        setDaysInput("1"); setDays(1); setActiveDay(1);
                      } else if (v > 30) {
                        setDaysInput("30"); setDays(30);
                      } else {
                    ...
                    >
                      ...
                      {startDate ? (
                        <>
                          ...
                          <span className="iw-day-tab-date">{dateForDayLocal(d)}</span>
                        </>
                      ) : (
                        `Day ${d}`
                      )}
...
const styles = `
  ...
  /* Date input — force calendar icon to be visible on dark bg */
  .iw-input--date {
    color-scheme: dark;
    cursor: pointer;
  }
  .iw-input--date::-webkit-calendar-picker-indicator {
    filter: invert(0.7) sepia(1) saturate(2) hue-rotate(200deg);
    cursor: pointer;
    opacity: 0.8;
  }
  .iw-input--date::-webkit-calendar-picker-indicator:hover {
    opacity: 1;
  }

  /* Number input — remove spinner arrows that fight with manual typing */
