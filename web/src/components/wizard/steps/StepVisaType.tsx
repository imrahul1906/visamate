...
// Icon map — keyed by visa type code from JSON (uppercase)
...
const VISA_DESCRIPTIONS: Record<string, string> = {
  TOURIST: "For tourism, sightseeing, visiting friends or family",
  BUSINESS: "For business meetings, conferences, trade, etc.",
  TRANSIT: "For passing through the country",
  STUDENT: "For studying in educational institutions",
  WORK: "For employment in the country",
};
...
export default function StepVisaType({ countryCode, selectedVisa, onSelect, compact }: Props) {
  ...
  if (loading) {
    return (
      <div className="py-8 text-center text-sm text-gray-400">Loading visa types...</div>
    );
  }
  ...
  if (visaTypes.length === 0) {
    return (
      <div className="py-8 text-center text-sm text-gray-400">
        No visa types found for the selected country.
      </div>
    );
  }
      ...
      <div className="space-y-2">
        {visaTypes.map((option) => {
          ...
          return (
            <div
              key={option.id}
              onClick={() => onSelect(isActive ? null : option.code, isActive ? null : option.name)}
